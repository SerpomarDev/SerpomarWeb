...
document.addEventListener('DOMContentLoaded', (event) => {
        ...
        const myDropzone = new Dropzone('#SaveFile', {
            ...
            init: function() {
                this.on('addedfile', async function(file) {
                    ...
                    let filePath = `adjuntos_conductores/${id}/${fileName}`;
                    const storageRef = storage.ref(filePath);

                    // Check if the file already exists
                    const existingFiles = await storage.ref(`adjuntos_conductores/${id}`).listAll();
                    ...
                    if (fileNames.includes(fileName)) {
                        ...
                        filePath = `adjuntos_conductores/${id}/${fileName}`;
                    }
    ...
    async function loadUploadedFiles(id) {
        ...
        try {
            const listRef = storage.ref(`adjuntos_conductores/${id}`);
