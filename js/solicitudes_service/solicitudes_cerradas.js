...
// fetch("https://esenttiapp-production.up.railway.app/api/solicitudservclosed",{
//   headers: {
...
fetch("https://esenttiapp-production.up.railway.app/api/solicitudservclosed", {
  ...
  .then(response => response.json())
  .then(data => {
      const processedData = data.map(SoliClosed => {
          return {
              id_primario: SoliClosed.id_primario,
              do_sp: SoliClosed.do_sp,
              do_pedido: SoliClosed.do_pedido,
              pedido: SoliClosed.pedido,
              contenedor: SoliClosed.contenedor,
              impexp: SoliClosed.impexp,
              cliente: SoliClosed.cliente,
              fecha_actualizacion: SoliClosed.fecha_actualizacion,
              pendiente_liquidar: SoliClosed.pendiente_liquidar
          };
      });
