...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    async function uploadId(id) {
        document.getElementById('id_humana').value = id;
        modal.style.display = 'block';
        ...
        const myDropzone = new Dropzone('#SaveFile', {
            ...
            init: function() {
                this.on('addedfile', async function(file) {
                    const id = document.getElementById('id_humana').value;
    ...
    async function deleteFile(id, fileName) {
        try {
            const fileRef = storage.ref(`gestionHumana/${id}/${fileName}`); // Updated storage path
    ...
    async function loadUploadedFiles(id) {
        ...
        try {
            const listRef = storage.ref(`gestionHumana/${id}`); // Updated storage path
