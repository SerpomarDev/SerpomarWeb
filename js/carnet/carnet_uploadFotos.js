...
document.addEventListener('DOMContentLoaded', (event) => {
        ...
        const myDropzone = new Dropzone('#SaveFile', {
            ...
            init: function() {
                this.on('addedfile', async function(file) {
                    ...
                    let filePath = `foto_carnet/${id}/${fileName}`;
                    const storageRef = storage.ref(filePath);

                    // Check if the file already exists
                    const existingFiles = await storage.ref(`foto_carnet/${id}`).listAll();
                    ...
                    if (fileNames.includes(fileName)) {
                        ...
                        filePath = `foto_carnet/${id}/${fileName}`;
                    }
    ...
    async function deleteFile(id, fileName) {
        try {
            const fileRef = storage.ref(`foto_carnet/${id}/${fileName}`); // Updated storage path
    ...
    async function loadUploadedFiles(id) {
        ...
        try {
            const listRef = storage.ref(`foto_carnet/${id}`); // Updated storage path
