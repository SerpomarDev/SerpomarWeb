...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    async function uploadId(id) {
        document.getElementById('id_ordenc').value = id;
        modal.style.display = 'block';
        ...
        const myDropzone = new Dropzone('#SaveFile', {
            ...
            init: function() {
                this.on('addedfile', async function(file) {
                    const id = document.getElementById('id_ordenc').value;
                    let fileName = file.name;
                    let filePath = `ordenCompra/${id}/${fileName}`;
                    const storageRef = storage.ref(filePath);

                    // Check if the file already exists
                    const existingFiles = await storage.ref(`ordenCompra/${id}`).listAll();
                    ...
                    if (fileNames.includes(fileName)) {
                        ...
                        filePath = `ordenCompra/${id}/${fileName}`;
                    }
    ...
    async function deleteFile(id, fileName) {
        try {
            const fileRef = storage.ref(`ordenCompra/${id}/${fileName}`); // Updated storage path
    ...
    async function loadUploadedFiles(id) {
        ...
        try {
            const listRef = storage.ref(`ordenCompra/${id}`); // Updated storage path
