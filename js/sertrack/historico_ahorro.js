fetch('https://esenttiapp-production.up.railway.app/api/ahorrobycliente', {
  ...
  .then(apiData => {
    ...
    // Obtener la lista de meses (excluyendo Octubre)
    const mesesRestantes = Object.keys(groupedData).filter(month => month !== 'Octubre');

    // Ordenar los meses restantes cronológicamente
    const monthOrder = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Noviembre", "Diciembre"];
    mesesRestantes.sort((a, b) => monthOrder.indexOf(a) - monthOrder.indexOf(b));

    // Unir "Octubre" con los meses restantes ordenados
    const labels = ['Octubre', ...mesesRestantes];
