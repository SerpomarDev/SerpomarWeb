  ...
  function filtrarYContarFechasCita() {
    ...
    .then(data => {
      ...
      data.forEach(item => {
        ...
        if (item.fecha_cliente && item.cliente === clienteFiltrar) {
          const fechaCita = item.fecha_cliente.slice(0, 10);
      ...
      document.getElementById('fecha_cliente').textContent = contador;
    })
    .catch(error => {
